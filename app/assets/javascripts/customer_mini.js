    ...
    const desc_privacy = () => {
      ...
      $('#private_bottom_base').html(
        ...
        <div class="row">
          ...
          <div class="cont">${obj.min_sdate.str_date(0)}</div>
        </div>
        <div class="row">
          <div class="indi">最終来院日</div>
          <div class="cont">${obj.max_sdate.str_date(0)}</div>
        </div>
        `
      );
    }
    ...
    const desc_details = () => {
      ...
      objs.forEach((cell) => {
        if (date_current != cell.sdate) {
          ap += `<tr><th style="border-right:0;" colspan="4"></th></tr>`;
          date_current = cell.sdate;
        }

        let data = cell.data_0 + cell.data_1 + cell.data_2;
        ap +=
        `
        <tr>
          <th>${cell.sdate.str_date(0)}</th>
          <th>¥${data.toLocaleString()}</th>
          <td>${cell.product_name}</td>
          <td>
            <a href="/home_sp/2/1/true/${cell.staff_id}/false/0/0">${cell.staff_name}</a>
          </td>
        </tr>
        `;
      });
